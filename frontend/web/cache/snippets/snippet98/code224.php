<div class="poster {$bg-position}" style="background: {$background-color}; 
                         {if !empty(bg_image)} background-image: url({$bg_image}) {/if}" id="big-header">
  <div class="poster-content {$align}" style="color : {$text-color}">
	  <div class="main-box">
        <h1 style="color : {$h1-color}">{$nadpis}</h1>
        <h3>{$podnadpis}</h3>
        <ul>
