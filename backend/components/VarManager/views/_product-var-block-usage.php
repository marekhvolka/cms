...
list($pageAreas, $portalAreas, $products, $portals) = $block->getChildrenOwners();
...
<?php if (count($pageAreas)) : ?>
    ...
    <ul>
        <?php foreach ($pageAreas as $area) : ?>
            <li>
                <a href="<?= Url::to(['/page/edit', 'id' => $area[2]->id]) ?>#block-<?= $area[1]->id ?>"><?= $area[2]->breadcrumbs ?>
                    (<?= $area[0]->type ?>) - <?= $area[2]->portal->name ?></a>
            </li>
        <?php endforeach; ?>
    </ul>
<?php endif; ?>
